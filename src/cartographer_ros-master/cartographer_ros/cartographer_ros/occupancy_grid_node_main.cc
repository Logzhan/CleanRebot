...
Node::Node(const double resolution, const double publish_period_sec)
    : msg_ptr(nullptr),
      ...
      //ros::Subscriber sub = nh.subscribe<std_msgs::String>("chatter",10,doMsg);第二个是数据类型，第三个是回调函数
...
void Node::DrawAndPublish(const ::ros::WallTimerEvent& unused_timer_event) {
	...
	// 更新原始的语义地图
	Slices(submap_slices_, resolution_);
	auto painted_slices = PaintSubmapSlices(submap_slices_, resolution_);
	//zhanli@newAdd : get fusion sem map from function
	int *fusion_sem_map = cartographer::io::GetFusionSemMap();

	std::unique_ptr<nav_msgs::OccupancyGrid> msg_ptr = CreateOccupancyGridMsg(
		painted_slices, fusion_sem_map, resolution_, last_frame_id_, last_timestamp_);
	// 发送occupancy_gird地图
