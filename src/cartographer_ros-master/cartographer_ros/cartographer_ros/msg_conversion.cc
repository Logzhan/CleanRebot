  ...
  for (size_t i = 0; i < msg.ranges.size(); ++i) {
    ...
    if (HasEcho(echoes)) {
      ...
      if (msg.range_min <= first_echo && first_echo <= msg.range_max) {
        ...
        //point_cloud.intensities.push_back(127);
        // 如果存在强度信息
...
std::unique_ptr<nav_msgs::OccupancyGrid> CreateOccupancyGridMsg(
  ...
  for (int y = height - 1; y >= 0; --y) {
    for (int x = 0; x < width; ++x) {
      ...
      //if (packed!=(0xffffa500)&& packed!=(0xffffff00))
      //std::cout <<std::hex<< packed << "w   ";
      // 根据packed获取像素值[0-255]
       const unsigned char co =  packed >> 24;
       //if(co!=255)
       //std::cout <<int (co) << "w   ";
      const unsigned char color =  packed >> 16;
      // 根据packed获取这个栅格是否被更新过
      const unsigned char observed = packed >> 8;
      // 。。。const unsigned char color = packed;
...
std::unique_ptr<nav_msgs::OccupancyGrid> CreateOccupancyGridMsg(
	...
	occupancy_grid->info.width = width;
	occupancy_grid->info.height = height;
	occupancy_grid->info.origin.position.x = -1000 * resolution;                      
	occupancy_grid->info.origin.position.y = (-1000) * resolution;

	std::cout<<(occupancy_grid->info.origin.position.x) <<"x1    "<< occupancy_grid->info.origin.position.y<<"y1   "<<std::endl; //map坐标系偏移
	// 对于二维地图高度为0
	occupancy_grid->info.origin.position.z = 0.;
	// 旋转量设置为0
	occupancy_grid->info.origin.orientation.w = 1.;
	...
	occupancy_grid->data.reserve(width * height);
	// 更新发送整个语义地图，添加数据到occupancy_grid中
	for (int y = height - 1; y >= 0; --y) {
		for (int x = 0; x < width; ++x) {
			// 根据packed获取这个栅格是否被更新过
			const int sem = pixel_data_color[y * width + x];
			//如果value>65,将color赋给删去const的value 
