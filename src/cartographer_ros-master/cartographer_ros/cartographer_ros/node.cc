...
Node::Node(
  ...
  // submap_list发布器，用于发布submap_list信息
...
void Node::PublishSubmapList(const ::ros::WallTimerEvent& unused_timer_event) {
	absl::MutexLock lock(&mutex_);
	// 从map_builder_bridge中获取submapLis并发送
	submap_list_publisher_.publish(map_builder_bridge_.GetSubmapList());
}

int Node::PublishText() {
...
void Node::LaunchSubscribers(const TrajectoryOptions& options,
    ...
    // ::ros::Subscriber sub_map = node_handle_.subscribe<::nav_msgs::OccupancyGrid>("map", 1, 
    //       boost::function<void(const nav_msgs::OccupancyGrid::ConstPtr& )>(
    //       [this](const nav_msgs::OccupancyGrid::ConstPtr& msg) {
    //         (this->HandleMap)(msg);
    //       }));
	subscribers_[trajectory_id].push_back({node_handle_.subscribe<::nav_msgs::OccupancyGrid>("map", 1, 
	          boost::function<void(const nav_msgs::OccupancyGrid::ConstPtr& )>(
	          [this](const nav_msgs::OccupancyGrid::ConstPtr& msg) {
	            (this->HandleMap)(msg);
	          })), "map_wyk"});
